...
function removeActiveViews (viewMask) {
  const mask = {}
  for (const k of Object.keys(viewMask)) {
    if (viewMask[k] === 2 || viewMask[k] === 1) { mask[k] = 1 } else viewMask[k] = 0
  }
  return mask
}
...
//  let mask = makeNewViewMask(state.tiles)
  const mask = addViewMasks(state, makeNewViewMask(state.tiles), removeActiveViews(state.playerData[player].viewMask))
