...
function makeConfigFromMenu () {
  const online = menuData.NewGameData.Online
  const config = {}
  ...
  if (online) {
    const tempPlayerlist = menuData.OnlinePlayers.map(x => {
      if (x.PlayerType === 'None' || x.PlayerType === 'AI') { return x } else { return { PlayerType: 'Human', Alliance: x.Alliance } }
    })
...
function onTechHexClicked (hex) {
  const tech = data.techs.find(t => t.hex.compare(hex))
...
function onMenuHexClicked (hex) {
  ...
  if (menuData.Screen === 'MainMenu') {
    ...
    const opt = data.newGameMenu.find(t => t.hex.compare(hex))
    const action = opt.name
    console.log(opt)
    ...
    switch (action) {
      ...
      case 'PlayerType':
        ...
        break
      case 'Alliance':
        if (menuData.NewGameData.Online) { menuData.OnlinePlayers[opt.num].Alliance = menuData.OnlinePlayers[opt.num].Alliance % 6 + 1 } else { menuData.OfflinePlayers[opt.num].Alliance = menuData.OfflinePlayers[opt.num].Alliance % 6 + 1 }
        break
    }
  }
  drawScreen()
...
function quickSetup () {
  const config = { numPlayers: 4, boardSize: 8, numHumans: 2, playersTogether: false }
